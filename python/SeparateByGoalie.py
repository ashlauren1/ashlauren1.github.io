...
gamelogs_goalies_df = pd.read_csv(os.path.join(input_folder, 'gamelogs_goalies.csv'), encoding='utf-8')
totals_goalies_df = pd.read_csv(os.path.join(input_folder, 'seasontotals_goalies.csv'), encoding='utf-8')
...
def create_totals_table(df, player_id):
    ...
    table_html = '<table id="totals">\n<tr><th>Team</th><th>Goalie</th><th>GP</th><th>G</th><th>A</th><th>PTS</th><th>SOG</th><th>HIT</th><th>BLK</th><th>PIM</th><th>TOI</th><th>ATOI</th><th>S%</th><th>EVG</th><th>PPG</th><th>SHG</th><th>GWG</th><th>EVA</th><th>PPA</th><th>SHA</th></tr>\n'
    for _, row in totals_df.iterrows():
        team_link = create_team_link(row['Team'])
        player_link = create_player_link(row['Player'], row['PlayerID'])
        table_html += f"<tr><td>{team_link}</td><td>{player_link}</td><td>{row['GP']}</td><td>{row['GA']}</td><td>{row['GAA']}</td><td>{row['SA']}</td><td>{row['SV']}</td><td>{row['SV%']}</td><td>{row['Shutouts']}</td><td>{row['W']}</td><td>{row['L']}</td><td>{row['OT/SOL']}</td><td>{row['TOI']}</td><td>{row['ATOI']}</td></tr>\n"
    table_html += '</table>\n'
    return table_html

# Function to create the game log table for a specific player
def create_goalies_gamelog_table(df, player_id):
    goalies_df = df[df['PlayerID'] == player_id]
    table_html = f'<table id="{player_id}_gamelog">\n<tr><th>Date</th><th>Player</th><th>Team</th><th>Location</th><th>Opp</th><th>Result</th><th>OT/SO</th><th>GA</th><th>SA</th><th>SV</th><th>SV%</th><th>PIM</th><th>TOI</th></tr>\n'
    for _, row in goalies_df.iterrows():
        game_link = create_game_link(row['Date'], row['GameID'])
        player_link = create_player_link(row['Player'], row['PlayerID'])
        team_link = create_team_link(row['Team'])
        opp_link = create_opp_link(row['Opp'])
        table_html += f"<tr><td>{game_link}</td><td>{player_link}</td><td>{team_link}</td><td>{row['Location']}</td><td>{opp_link}</td><td>{row['Result']}</td><td>{row['OT/SO']}</td><td>{row['GA']}</td><td>{row['SA']}</td><td>{row['SV']}</td><td>{row['SV%']}</td><td>{row['PIM']}</td><td>{row['TOI']}</td></tr>\n"
    table_html += '</table>\n'
    return table_html
...
unique_players = pd.concat([gamelogs_goalies_df[['PlayerID', 'Player']], totals_goalies_df[['PlayerID', 'Player']]]).drop_duplicates()
...
for _, player_row in unique_players.iterrows():
    ...
    goalies_total_table = create_totals_table(totals_goalies_df, player_id)
    goalies_gamelog_table = create_goalies_gamelog_table(gamelogs_goalies_df, player_id)

    # Combine tables and write to the player's HTML file
    tables = [goalies_total_table, goalies_gamelog_table]
